...
Partial Class resIngreso
    ...
    Protected Sub btnIngresar_Click(sender As Object, e As System.EventArgs) Handles btnIngresar.Click
        ...
        cmd.CommandText = " SELECT ID,USUARIO,CODIGO,DEFPASS FROM SOL_PEDIDOS.PEDIDOS.ACCESO WHERE MODULO='R' AND ESTADO='H' AND USUARIO=@usuario"
        ...
        If (dr.Read) Then
            ...
            id = CStr(dr(0))
            Session.Add("proyecto", CStr(dr(1)))
            Session.Add("codigo", CStr(dr(2)))
            Session.Add("defPass", CStr(dr(3)))
            Session.Add("id", id)
            query = "SELECT TOP 1 PASS FROM SOL_PEDIDOS.PEDIDOS.PASSWORD WHERE ID_USUARIO='" & id & "' ORDER BY FECHA_EXP DESC"
        ...
        Else
            ...
            lblMensaje.Text = "Nombre de proyecto inexistente"
        End If
