...
Partial Class ingenieria_nuevaActa
    ...
    Function validarIngreso() As Boolean 'si devuelve TRUE significa que existen registros con los mismos valores
        ...
        query += " WHERE COD_ACTA='" & lblCodigoActa.Text & "'"
    ...
    Sub validarReimpresionActa()
        Dim cont As Integer
        query = "SELECT COUNT(*) FROM SOL_PEDIDOS.PEDIDOS.ACTAS_CABECERA WHERE CODIGO_ACTA='" & lblCodigoActa.Text & "'"
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        lblCodigoActa.Text = Session("acta")
        validarReimpresionActa()
    ...
    Protected Sub btnAgregar_Click(sender As Object, e As EventArgs) Handles btnAgregar.Click
        ...
        Dim articulo As String = obtenerArticulo()
        Dim codActa As String = lblCodigoActa.Text
        ...
        If lblId.Text = "0" Then
            ...
            query += " (PROYECTO,ACTIVIDAD,FASE,ARTICULO,CANTIDAD,CODIGO_ACTA,CODIGO_CONTROL,UND_MED,UND_MED_A,CANTIDAD_APS)"
            query += " VALUES "
            query += " ('" & proyecto & "','" & actividad & "','" & material & "','" & articulo & "','" & cantidadEq & "','" & codActa & "', 'C','" & um & "','" & umEq & "','" & cantidad & "')"
    ...
    Protected Sub btnGenerarAceptar_Click(sender As Object, e As System.EventArgs) Handles btnGenerarAceptar.ServerClick
        ...
        If txtCodigoActa.Value.ToString = "" Then
            lblMensajeS.Attributes("Style") = ""
            lblMensajeS.Attributes("class") = "alert alert-danger"
        ...
        query = " INSERT INTO SOL_PEDIDOS.PEDIDOS.ACTAS_CABECERA "
        query += " (CODIGO_ACTA, ESTADO, FECHA,CODIGO_ACTA_ING, FECHA_ACTA_ING, RecordDate, UpdatedBy)"
        query += " VALUES "
        query += " ('" & lblCodigoActa.Text & "','A',GETDATE(),'" & txtCodigoActa.Value.ToString & "', '" & FechaActa.ToString("yyyy/MM/dd") & "', GETDATE(),'" & lblUsuario.Text & "')"
        Try
            fn.ejecutarComandoSQL(query, lblMensaje, lblMensajeS)
        Catch ex As Exception
    ...
    Protected Sub btnDescartar_Click(sender As Object, e As System.EventArgs) Handles btnDescartar.ServerClick
        query = " SELECT count('a') FROM SOL_PEDIDOS.PEDIDOS.ACTAS_CABECERA"
        query += " WHERE CODIGO_ACTA = '" & lblCodigoActa.Text & "'"

        Dim cant As Integer = CInt(fn.DevolverDatoQuery(query))

        If cant = 0 Then

            query = "UPDATE SOL_PEDIDOS.PEDIDOS.ACTAS_LINEA SET CODIGO_CONTROL='B', UpdatedBy='" & lblUsuario.Text & "', RecordDate=GETDATE()  "
            query += " WHERE CODIGO_ACTA='" & lblCodigoActa.Text & "'"
            fn.ejecutarComandoSQL(query, lblMensaje, lblMensajeS)
            Response.Redirect("Principal.aspx")
        Else
